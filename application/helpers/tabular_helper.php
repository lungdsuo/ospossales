...
function get_sales_manage_table_headers()
{
	...
	if($CI->config->item('invoice_enable') == TRUE)
	{
		...
		$headers[] = array('invoice_no_gst' => '&nbsp', 'sortable' => FALSE);
	}
...
function get_sale_data_row($sale)
{
	...
	if($CI->config->item('invoice_enable'))
	{
		...
		$row['invoice_no_gst'] = empty($sale->invoice_number) ? '' : anchor($controller_name."/invoicewithoutgst/$sale->sale_id", '<span class="glyphicon glyphicon-hourglass"></span>',
			array('title'=> "Non GST Invoice")
		);
